...
- `ui/` — screens and widgets (Kor look, see **UI structure** below):
  ...
  - `components/` — `ReminderCard`, `BirthdayCard`, `BirthdayRow`, `SectionHeader`, `GroupedCard`,
...
- **Takvim and Doğum günleri (F4.4):** pure date math in `domain/calendar_dates.dart`
  ...
  (strip, grid, "Bugün") re-bases the agenda on it. Week strip pages with a
  `PageView` (chevrons are the button alternative), the grid opens with ▦ or a pull
  on the handle. **Reschedule:** `AgendaReminderRow` wraps `ReminderCard` in a
