...
- `domain/model/subtask.dart` — `Subtask { id, title, isDone, position }` (F3.3) and
  ...
  `;` and a separate-word " ve " (bullets/checkboxes stripped) — the future quick
  capture "Maddelere böl" (§3.3.3); `splitSubtaskLines` splits line breaks only
  (paste, Enter). Completing a reminder never completes its subtasks and all
