...
- **CI maliyeti (private repo):** Depo **private**, yani Actions dakikaları ölçülüyor (ücretsiz olması public olmaya bağlıydı). `CI` (format + analyze + test) her kod PR'ında çalışır — ucuz ve asıl güvenlik ağı. `Android build` (release/R8 + birleşik manifest denetimi) yalnızca `android/**`, pubspec veya kendi iş akışı değişince çalışır. **`iOS build` yalnızca elle** başlatılır: macOS koşucuları **10×** faturalanır, yani 4 dakikalık bir derleme kotadan ~40 dakika yer. `ios/**`, bağımlılık veya platform kanalı değişince Actions sekmesinden elle çalıştırın. Merge sonrası `master`'da otomatik CI çalışmaz. PR başına gereksiz push'tan kaçının (her push CI'ı yeniden başlatır).
  *Emülatör testleri (`e2e.yml`, PR #55, henüz merge edilmedi):* bir koşu ~25–30 dakika. Merge edilirken `pull_request` ve gecelik `schedule` tetikleyicileri **kaldırılıp yalnızca `workflow_dispatch`** bırakılmalı; aksi halde her PR yarım saat kota yer.
