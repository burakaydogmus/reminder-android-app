...
- [x] **F0.4 Cihaz üstü uçtan uca testler** · `test/emulator-e2e` · *bağımlı: F0.2*
  `integration_test/` + `.github/workflows/e2e.yml`: gerçek Android emülatöründe (`reactivecircus/android-emulator-runner`, API 34 `google_apis` x86_64, KVM, AVD snapshot önbelleği) gerçek uygulamayı sürer. `test/` altındaki ~2750 test Dart test ana bilgisayarında çalışır — orada `Platform.isAndroid` `false`, bildirim eklentisi sahte, SQLite bellekte, `home_widget` bir kabuk; yani **platform bütünleşmelerinin hiçbiri hiç çalışmamıştı**. Kapsanan: soğuk açılış + onboarding, `drift`/`sqlite3` dosyasına yazma ve **gerçek süreç yeniden başlatmasından** sonra okuma, `pendingNotificationRequests()` ile gerçek bildirim kurulumu (tek seferlik / tekrarlayan / doğum günü, F6.2c tam zamanlı alarm yedeği iki durumda da), `widget_payload_v2` deposu, gerçek dosya sistemi üzerinde yedek turu, `reminderwidget://` yönlendirmesi ve F3.7 rutin uygulama. Native tarafta adb ile: kısayolların (F5.3) ve widget sağlayıcılarının (F5.1) gerçekten yayımlanması, derin bağlantı intent'lerinin `MainActivity`'ye ulaşması. İş akışı yalnızca `lib/**`, `android/**`, `integration_test/**`, pubspec değişince (dokümana özel PR'larda hiç çalışmaz), ayrıca gecelik ve elle. **Kapsanmayan:** iOS (Mac yok), API 26–30 davranışı, bildirimin gerçekten çalması ve bildirim aksiyonları, geofence tetiklenmesi, paylaşım/dosya seçici sistem arayüzü, widget'ın ekrandaki görüntüsü. Ayrıntı: `CLAUDE.md` → **Cihaz üstü uçtan uca testler**.
  *Not:* madde Faz 0'a ait (test altyapısı) ama Faz 8'den sonra yazıldı: test edeceği platform yolları (widget, rutin, yedek, izinler) ancak o zaman vardı.
